...
Parser.prototype.write = function(data) {
  ...
  do {
    // console.log('do:', '"' + String.fromCharCode(data[pos]) + '"', data[pos], 'REPLY_TYPE_' + this.types[reply], 'TYPE_' + this.types[type], 'STATE_' + this.states[state], '"' + value + '"', 'value:', value.length + '/' + bulkLength, 'multi:', multi.length + '/' + multiLength);
    switch (state) {
      case STATE_INIT:
        c = data[pos++];
        reply = 0;
